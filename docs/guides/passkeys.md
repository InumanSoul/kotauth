...
Two toggles on the workspace security page (`/admin/workspaces/<slug>/security`):

- **Passkey sign-in enabled** (default: on) — controls whether users can enroll and sign in with passkeys.
- **Disable password sign-in** (default: off) — when checked, the tenant becomes passwordless-only. Users sign in with a passkey or a magic-link email.
...
Users enroll passkeys via `/t/<slug>/account/passkeys` on the portal. Each device is a separate credential — enrolling on a phone doesn't cover the laptop; enroll multiple.
...
Operators can view and revoke individual credentials from the per-user detail page:
...
## Known limitations (v1.20.0)

- **Enterprise attestation is off.** All authenticators are accepted (`attestation=none`). FIDO MDS blocklist verification is a v1.20.1 candidate.
- **User verification is `preferred`.** Modern platform authenticators always provide it; older U2F-only hardware keys can enroll without. Per-tenant `required` override is a v1.20.1 candidate.
