...
- **Generated URLs now respect `X-Forwarded-*` headers from the reverse proxy** — `install(XForwardedHeaders)` added to the Ktor module. Without it, Kotauth running behind a TLS-terminating proxy (OrbStack locally, nginx/Cloudflare/ALB in production) saw every request as `http://<internal-host>:8080` because TLS termination happens in front. The OIDC discovery document consequently advertised `http://` URLs for `issuer`, `token_endpoint`, `userinfo_endpoint`, etc. SPAs that hit the HTTP URL were auto-redirected by the proxy to HTTPS, but the proxy-emitted 301 response carried no CORS headers, so the browser blocked the request mid-chain. Same bug also silently broke invite-link, password-reset, and email-verification URLs sent over email. **Security note:** `XForwardedHeaders` trusts these headers unconditionally; only safe when Kotauth is always behind a proxy that strips client-supplied `X-Forwarded-*` and sets its own (the documented deployment model)
