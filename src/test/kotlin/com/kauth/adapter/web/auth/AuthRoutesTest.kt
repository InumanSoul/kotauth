...
class AuthRoutesTest {
    ...
    private val user = User(
        ...
        passwordHash = hasher.hash("correct-pass"), enabled = true
    )

    private val publicApp = Application(
    ...
    private fun resetFixtures() {
        tenantRepo.clear(); userRepo.clear(); appRepo.clear()
        authCodeRepo.clear(); sessionRepo.clear(); auditLog.clear()
        tenantRepo.add(tenant); userRepo.add(user); appRepo.add(publicApp)
    }
    ...
    @Test
    fun `POST login redirects to mfa-challenge when user has MFA enabled`() = testApplication {
        resetFixtures()
        ...
        assertTrue(setCookie.any { it.contains("KOTAUTH_MFA_PENDING") },
    ...
    @Test
    fun `POST login redirects to forgot-password with reason=expired for expired password`() = testApplication {
        resetFixtures()
        // Seed a user with an expired password
        userRepo.clear()
        userRepo.add(user.copy(
            lastPasswordChangeAt = Instant.now().minusSeconds(100L * 86_400)
        ))
        val expiredTenant = tenant.copy(passwordPolicyMaxAgeDays = 90)
        tenantRepo.clear(); tenantRepo.add(expiredTenant)
    ...
    @Test
    fun `GET mfa-challenge redirects to login when KOTAUTH_MFA_PENDING cookie is absent`() = testApplication {
        resetFixtures()
    ...
    @Test
    fun `GET mfa-challenge redirects to login when cookie signature is invalid`() = testApplication {
        resetFixtures()
    ...
    @Test
    fun `GET mfa-challenge shows challenge form when valid signed cookie is present`() = testApplication {
        resetFixtures()
    ...
    @Test
    fun `GET auth endpoint returns 400 for unsupported response_type`() = testApplication {
        resetFixtures()
    ...
    @Test
    fun `GET auth endpoint returns 400 when client_id is missing`() = testApplication {
        resetFixtures()
    ...
    @Test
    fun `GET auth endpoint renders login page for valid OAuth2 params`() = testApplication {
        resetFixtures()
        ...
        assertEquals(HttpStatusCode.OK, response.status)
        val body = response.bodyAsText()
        assertTrue(body.contains("form") || body.contains("login"),
    ...
    @Test
    fun `POST token endpoint returns JSON token set on valid authorization_code exchange`() = testApplication {
        resetFixtures()
        val oauthSvc = buildOAuthService()

        // Pre-issue a valid authorization code (simulates a completed login)
        val issueResult = oauthSvc.issueAuthorizationCode(
    ...
    @Test
    fun `POST token endpoint returns 400 for unsupported grant_type`() = testApplication {
        resetFixtures()
