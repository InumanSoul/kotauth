...
internal fun Route.emailOtpLoginRoutes(
    ...
    post("/email-otp/send") {
        handleSendOtp(call, emailOtpService, perIpLimiter, encryptionService, secure)
    }
    ...
    post("/email-otp/verify") {
        handleVerifyOtp(call, emailOtpService, oauthService, encryptionService, ssoTtlSeconds, secure)
    }
}

private suspend fun handleSendOtp(
    ...
    val ctx = call.attributes[AuthTenantAttr]
    if (ctx.tenant?.securityConfig?.emailOtpLoginEnabled != true) {
        return call.respondRedirect("/t/${ctx.slug}/account/login")
    }
    ...
    if (!perIpLimiter.isAllowed("$PER_IP_RATE_PREFIX:$ipAddress")) {
        return call.respondRedirect("/t/${ctx.slug}/email-otp/verify${if (isResend) "?resent=true" else ""}")
    }
...
private suspend fun handleVerifyOtp(
    ...
    val otp = call.getOtpFlowContext(encryptionService)
    if (otp == null) {
        return call.respondRedirect("/t/${ctx.slug}/email-otp")
    }
...
private fun OtpFlowContext.errorMessageFor(error: OtpError): String =
    when (error) {
