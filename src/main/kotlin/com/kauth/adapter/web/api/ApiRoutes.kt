...
fun Route.apiRoutes(
    ...
    get("/api/docs/openapi.yaml") {
        val spec = ApiRoutes::class.java.getResourceAsStream("/openapi/v1.yaml")
    ...
    authenticate("api-key") {
        route("/t/{tenantSlug}/api/v1") {
            ...
            intercept(ApplicationCallPipeline.Call) {
                ...
                val resolvedKey = apiKeyService.validate(principal.rawToken, tenant.id)
                    ?: return@intercept call.respondProblem(
                        status = HttpStatusCode.Unauthorized,
            ...
            route("/users") {
                ...
                post {
                    ...
                    when (val result = adminService.createUser(
                        tenantId = tenantId,
                ...
                route("/{userId}") {
                    ...
                    get {
                        ...
                        val userId   = call.parameters["userId"]?.toIntOrNull()
                            ?: return@get call.respondProblem(HttpStatusCode.BadRequest, "Invalid user ID", "userId must be an integer.")
                        val user = userRepository.findById(userId)
                            ?: return@get call.respondProblem(HttpStatusCode.NotFound, "User not found", "No user with id $userId in this workspace.")
                    ...
                    put {
                        ...
                        val tenantId = call.attributes[TenantIdAttr]
                        val userId   = call.parameters["userId"]?.toIntOrNull()
                            ?: return@put call.respondProblem(HttpStatusCode.BadRequest, "Invalid user ID", "userId must be an integer.")
                    ...
                    delete {
                        ...
                        val tenantId = call.attributes[TenantIdAttr]
                        val userId   = call.parameters["userId"]?.toIntOrNull()
                            ?: return@delete call.respondProblem(HttpStatusCode.BadRequest, "Invalid user ID", "userId must be an integer.")
                    ...
                    route("/roles") {
                        ...
                        delete("/{roleId}") {
                            ...
                            val tenantId = call.attributes[TenantIdAttr]
                            val userId   = call.parameters["userId"]?.toIntOrNull()
                                ?: return@delete call.respondProblem(HttpStatusCode.BadRequest, "Invalid user ID", "")
                            val roleId   = call.parameters["roleId"]?.toIntOrNull()
            ...
            route("/roles") {
                ...
                post {
                    ...
                    val roleScope = body.scope?.let { s -> RoleScope.entries.firstOrNull { it.value == s.lowercase() } } ?: RoleScope.TENANT
                    when (val result = roleGroupService.createRole(tenantId, body.name, body.description, roleScope, null)) {
                        is AdminResult.Success -> call.respond(HttpStatusCode.Created, result.value.toApiDto())
                        is AdminResult.Failure -> call.respondAdminError(result.error)
                    }
                }
                ...
                route("/{roleId}") {
                    ...
                    get {
                        ...
                        val role = roleRepository.findById(roleId)
                            ?: return@get call.respondProblem(HttpStatusCode.NotFound, "Role not found", "No role with id $roleId.")
            ...
            route("/groups") {
                ...
                post {
                    ...
                    when (val result = roleGroupService.createGroup(tenantId, body.name, body.description, body.parentGroupId)) {
                        is AdminResult.Success -> call.respond(HttpStatusCode.Created, result.value.toApiDto())
                        is AdminResult.Failure -> call.respondAdminError(result.error)
                    }
                }
                ...
                route("/{groupId}") {
                    ...
                    get {
                        ...
                        val group = groupRepository.findById(groupId)
                            ?: return@get call.respondProblem(HttpStatusCode.NotFound, "Group not found", "No group with id $groupId.")
                    ...
                    put {
                        ...
                        when (val result = roleGroupService.updateGroup(groupId, tenantId, body.name, body.description)) {
                            is AdminResult.Success -> call.respond(HttpStatusCode.OK, result.value.toApiDto())
                            is AdminResult.Failure -> call.respondAdminError(result.error)
                        }
                    }
                    ...
                    route("/members") {
                        ...
                        post("/{userId}") {
                            ...
                            val tenantId = call.attributes[TenantIdAttr]
                            val groupId  = call.parameters["groupId"]?.toIntOrNull()
                                ?: return@post call.respondProblem(HttpStatusCode.BadRequest, "Invalid group ID", "")
                            val userId   = call.parameters["userId"]?.toIntOrNull()
                                ?: return@post call.respondProblem(HttpStatusCode.BadRequest, "Invalid user ID", "")
            ...
            route("/applications") {
                ...
                get("/{appId}") {
                    ...
                    val app = applicationRepository.findById(appId)
                        ?: return@get call.respondProblem(HttpStatusCode.NotFound, "Application not found", "No application with id $appId.")
                ...
                put("/{appId}") {
                    ...
                    when (val result = adminService.updateApplication(
                        appId        = appId,
                        tenantId     = tenantId,
                        name         = body.name,
                ...
                delete("/{appId}") {
                    ...
                    val tenantId = call.attributes[TenantIdAttr]
                    val appId    = call.parameters["appId"]?.toIntOrNull()
                        ?: return@delete call.respondProblem(HttpStatusCode.BadRequest, "Invalid application ID", "")
            ...
            route("/sessions") {
                ...
                delete("/{sessionId}") {
                    ...
                    val session = sessionRepository.findById(sessionId)
                        ?: return@delete call.respondProblem(HttpStatusCode.NotFound, "Session not found", "No session with id $sessionId.")
            ...
            route("/audit-logs") {
                ...
                get {
                    ...
                    val eventType = params["eventType"]?.let { name ->
...
private suspend fun requireScope(call: ApplicationCall, scope: String): Unit? {
    val key = call.attributes.getOrNull(ApiKeyAttr)
    ...
    if (scope !in key.scopes) {
        call.respondProblem(
            HttpStatusCode.Forbidden, "Insufficient scope",
...
private suspend fun ApplicationCall.respondAdminError(error: AdminError): Unit = when (error) {
    ...
    is AdminError.Validation -> respondProblem(HttpStatusCode.UnprocessableEntity, "Validation Error", error.message)
}
...
private fun com.kauth.domain.model.User.toApiDto() = UserDto(
    id            = id!!,
    username      = username,
    email         = email,
    fullName      = fullName,
...
private fun com.kauth.domain.model.Role.toApiDto() = RoleDto(
    id          = id!!,
    name        = name,
...
private fun com.kauth.domain.model.Group.toApiDto() = GroupDto(
    id            = id!!,
    name          = name,
    description   = description,
...
private fun com.kauth.domain.model.Application.toApiDto() = ApplicationDto(
    id           = id!!,
    clientId     = clientId,
    name         = name,
    description  = description,
...
private fun com.kauth.domain.model.Session.toApiDto() = SessionDto(
    id        = id!!,
    userId    = userId,
    clientId  = clientId,
    scopes    = scopes,
...
private fun com.kauth.domain.model.AuditEvent.toApiDto() = AuditEventDto(
