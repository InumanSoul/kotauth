...
class PostgresMfaRepository : MfaRepository {
    ...
    override fun findEnrollmentByUserId(userId: Int, method: String): MfaEnrollment? = transaction {
        MfaEnrollmentsTable.selectAll()
            .where {
    ...
    override fun findEnrollmentById(id: Int): MfaEnrollment? = transaction {
        MfaEnrollmentsTable.selectAll()
    ...
    override fun saveEnrollment(enrollment: MfaEnrollment): MfaEnrollment = transaction {
        val encryptedSecret = encryptSecret(enrollment.secret)
        val now = OffsetDateTime.now(ZoneOffset.UTC)

        val insertedId = MfaEnrollmentsTable.insert {
            ...
            it[createdAt]  = now
            it[verifiedAt] = enrollment.verifiedAt?.let { ts ->
    ...
    override fun updateEnrollment(enrollment: MfaEnrollment): MfaEnrollment = transaction {
    ...
    override fun deleteEnrollmentsByUser(userId: Int): Unit = transaction {
    ...
    override fun findUnusedRecoveryCodes(userId: Int): List<MfaRecoveryCode> = transaction {
        MfaRecoveryCodesTable.selectAll()
            .where {
    ...
    override fun saveRecoveryCodes(codes: List<MfaRecoveryCode>): Unit = transaction {
        val now = OffsetDateTime.now(ZoneOffset.UTC)
    ...
    override fun markRecoveryCodeUsed(codeId: Int): Unit = transaction {
        MfaRecoveryCodesTable.update({ MfaRecoveryCodesTable.id eq codeId }) {
            it[usedAt] = OffsetDateTime.now(ZoneOffset.UTC)
        }
    }

    override fun deleteRecoveryCodesByUser(userId: Int): Unit = transaction {
    ...
    private fun ResultRow.toRecoveryCode(): MfaRecoveryCode = MfaRecoveryCode(
